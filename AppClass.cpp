...
void Application::Display(void)
{
	...
	float fAnimationTIme = 2.0f;
	float fPercentage = fTime / fAnimationTIme;


	static float currentIndex = 0;
	if (currentIndex == m_stopsList.size()-1)
	{
		v3CurrentPos = glm::lerp(m_stopsList[currentIndex], m_stopsList[0], fPercentage);
		if (fPercentage >= 1.0f)
		{
			currentIndex = 0;
	...
	{
		v3CurrentPos = glm::lerp(m_stopsList[currentIndex], m_stopsList[currentIndex + 1], fPercentage);
		if (fPercentage >= 1.0f)
		{
			currentIndex++;
			dstartTime = GetTickCount();
